...
The current release is a playable one-level fluid-crowd test. The original one-million path retained one JavaScript `Enemy` object per unicorn, rebuilt crowd grids, wrote 64 bytes of instance data per live unit, and uploaded that range every frame. It fell below 1 FPS near 1,000,000 live units. The current path simulates exact integer population on a 64 × 36 fluid grid and maps every live unicorn point to its occupied cell through a small cumulative-population texture. Towers and missiles subtract real population from target cells, fortress arrivals remove population, and transfers only enter reachable non-wall cells. Tower attacks now draw short-lived cyan laser segments. Magic Missile draws an orange incoming streak, yellow impact rays, and a bright local light across the affected crowd. Effects are pooled and capped at 64. In an active Chrome test with 1,000,000 live points, fluid transfer, one firing tower, effect creation, cumulative-texture uploads, crowd illumination, and rendering enabled, the game measured 59.6 FPS over two seconds: 16.7 ms median, 17.5 ms p95, about 3.5 MB reported JS heap, and WebGL error zero. During that sample, the tower destroyed 105 unicorns, 112 reached the fortress, wall population remained zero, and `spawned = live + destroyed + breached` remained true. Tower, weapon, combat, wave, fire-rate, fluid conservation, wall, attack, arrival, million-cap, and effect checks pass.
