...
The current release is a one-level GPU crowd benchmark. The original one-million path retained one JavaScript `Enemy` object per unicorn, rebuilt crowd grids, wrote 64 bytes of instance data per live unit, and uploaded that range every frame. It fell below 1 FPS near 1,000,000 live units. The optimized path keeps only count, time, and running state on the CPU and generates positions procedurally from `gl_VertexID` in a `GL_POINTS` draw. Chrome measured 1,000,000 visible points at 59.8 FPS over two seconds: 16.7 ms median, 17.5 ms p95, about 2.6 MB reported JS heap, and WebGL error zero. This is a rendering and fluid-crowd baseline, not one million independently simulated or targetable enemies. Gameplay at this scale must resolve movement, health, tower damage, statuses, and knockback on aggregate density cells, then use those cells to drive the visual population. Tower, weapon, combat, wave, fire-rate, and constant-state million-crowd checks pass.
...
The user requested a temporary one-level experiment with 1,000,000 visible unicorns and no progression. Instancing alone did not solve the first version because the CPU still owned one object per unicorn and uploaded about 64 MB of transforms each frame. The current benchmark uses a normal 16,384-slot ECS world for the map and a separate GPU-procedural point pass for the crowd. Chrome reached the full 1,000,000 visual count at 59.8 FPS with WebGL error zero. The next gameplay-scale step is aggregate density/health/status cells plus a hybrid renderer: textured quads for nearby readable unicorns and procedural points or impostors for the distant mass.
