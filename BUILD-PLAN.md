...
The current release is a playable one-level fluid-crowd test. The original one-million path retained one JavaScript `Enemy` object per unicorn, rebuilt crowd grids, wrote 64 bytes of instance data per live unit, and uploaded that range every frame. It fell below 1 FPS near 1,000,000 live units. The current path simulates exact integer population on a 64 × 36 fluid grid and maps every live unicorn point to its occupied cell through a small cumulative-population texture. Towers and missiles subtract real population from target cells, fortress arrivals remove population, and transfers only enter reachable non-wall cells. Crowd movement now combines forward advection with conservative pressure relaxation and a minimum 64-unicorn movement packet. This fills corridor chambers broadly while preventing isolated unicorns from forming thin scout lines. In the repeatable 400,000-unit snapshot, the crowd covered 24 columns and 570 meaningful cells with only 2 sparse cells. In an active 1,000,000-unit Chrome sample it occupied 1,475 meaningful cells with only 5 sparse cells while measuring 59.6 FPS, 16.7 ms median, 17.5 ms p95, about 4.1 MB reported JS heap, zero wall population, exact conservation, and WebGL error zero. Tower attacks draw short-lived cyan laser segments. Magic Missile draws an orange incoming streak, yellow impact rays, and a bright local light across the affected crowd. Effects are pooled and capped at 64. Tower, weapon, combat, wave, fire-rate, fluid conservation, distribution, wall, attack, arrival, million-cap, and effect checks pass.
...
The user requested a temporary one-level experiment with 1,000,000 visible, destructible unicorns and no progression. Instancing alone did not solve the first version because the CPU still owned one object per unicorn and uploaded about 64 MB of transforms each frame. The current implementation follows the fluid approach: 2,304 cells store exact integer population, movement follows the shared flow field, blocked cells stay empty, and a cumulative texture expands cell populations into one million GPU points. Towers, missile blasts, and fortress arrivals modify those exact counts. Chrome measured the fully active one-million path at 60.0 FPS with WebGL error zero. Individual points are visual identities within a cell rather than persistent CPU entities; gameplay state is exact at cell resolution.
